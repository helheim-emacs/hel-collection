...
#+begin_quote
*NB:* Press =SPC= in any transient menu (including Magit ones) to show
hidden commands.
#+end_quote
...
- Diff and revert commands no longer have keys on the top layer — they’re
  moved to the same positions on the second layer under the =,= key.
